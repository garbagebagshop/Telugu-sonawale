...
export const InfoPage: React.FC<InfoPageProps> = ({ slug, onBack }) => {
  ...
  if (!page) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={onBack} className="underline font-bold">Back to Home</button>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-4xl mx-auto px-4 py-12">
      ...
      <header className="border-b-[3px] border-black pb-8 mb-8">
        <h1 className="text-4xl md:text-6xl font-bold mb-4 telugu-headline leading-tight">
          {page.title}
        </h1>
        {page.subtitle && (
          <p className="text-xl italic opacity-60 font-serif telugu-text">
            {page.subtitle}
          </p>
        )}
      </header>
      ...
      <div 
        className="telugu-text text-lg leading-relaxed space-y-6 prose prose-black max-w-none"
